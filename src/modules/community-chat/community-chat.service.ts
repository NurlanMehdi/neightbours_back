...
export class CommunityChatService {
  ...
  async sendMessage(userId: number, communityId: number, params: { text: string; replyToMessageId?: number }) {
    ...
    const authorName = [message.user?.firstName, message.user?.lastName].filter(Boolean).join(' ').trim() || 'User';
  ...
  async search(userId: number, query: string, communityId?: number, page = 1, limit = 50) {
  ...
  async deleteConversation(adminUserId: number, communityId: number) {
    const isAdmin = await this.repo.isAdmin(adminUserId);
    if (!isAdmin) throw new ForbiddenException('Только администратор может удалять чат');
    await this.repo.deleteChat(communityId);
  }

  async updateSettings(adminUserId: number, communityId: number, data: { isActive?: boolean; settings?: any }) {
    const isAdmin = await this.repo.isAdmin(adminUserId);
    if (!isAdmin) throw new ForbiddenException('Только администратор может изменять настройки');
