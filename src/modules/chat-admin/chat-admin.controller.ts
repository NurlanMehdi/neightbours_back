import { Controller, Delete, Get, Param, Query, UseGuards, Put, Body, ValidationPipe, Post } from '@nestjs/common';
import { ApiBearerAuth, ApiOperation, ApiQuery, ApiResponse, ApiTags, ApiBody, ApiParam } from '@nestjs/swagger';
...
export class ChatAdminController {
  ...
  @ApiOperation({ summary: 'Унифицированный список сообществ и событий для выпадающих списков (админ)' })
  @ApiQuery({ name: 'page', required: false, type: Number, example: 1, description: 'Номер страницы' })
  @ApiQuery({ name: 'limit', required: false, type: Number, example: 100, description: 'Количество элементов на странице' })
  @ApiQuery({ name: 'type', required: false, enum: ['COMMUNITY', 'EVENT'], description: 'Фильтр по типу (COMMUNITY или EVENT)' })
  ...
  @ApiQuery({ name: 'page', required: false, type: Number, example: 1, description: 'Номер страницы' })
  @ApiQuery({ name: 'limit', required: false, type: Number, example: 20, description: 'Количество элементов на странице' })
  @ApiQuery({ name: 'chatType', required: true, enum: ['COMMUNITY', 'EVENT'], description: 'Тип чата (обязательный)' })
  @ApiQuery({ name: 'chatId', required: true, type: Number, example: 1, description: 'ID чата (communityId или eventId) (обязательный)' })
  @ApiQuery({ name: 'userId', required: false, type: Number, example: 1, description: 'Фильтр по ID пользователя-автора' })
  @ApiQuery({ name: 'q', required: false, type: String, example: 'текст сообщения', description: 'Поиск по тексту сообщения' })
  @ApiResponse({ status: 200, type: ModerationMessagesResponseDto })
  async getModerationMessages(@Query() query: MessagesQueryDto): Promise<ModerationMessagesResponseDto> {
  ...
  @ApiParam({ name: 'id', type: Number, example: 1, description: 'ID сообщения для удаления' })
  @ApiQuery({ name: 'chatType', required: false, enum: ['COMMUNITY', 'EVENT'], description: 'Тип чата (COMMUNITY или EVENT)' })
  @ApiResponse({ status: 200, description: 'Помечено как удалённое', schema: { type: 'object', properties: { success: { type: 'boolean' }, id: { type: 'number' } } } })
  async softDelete(
  ...
  @ApiParam({ name: 'id', type: Number, example: 1, description: 'ID сообщения для одобрения' })
  ...
  @ApiParam({ name: 'id', type: Number, example: 1, description: 'ID сообщения для отклонения' })
  ...
  @ApiResponse({ status: 200, type: GlobalChatSettingsResponseDto })
  async updateGlobalChatSettings(@Body() body: any): Promise<GlobalChatSettingsResponseDto> {
    ...
    }).transform(payload, { type: 'body', metatype: UpdateGlobalChatSettingsDto as any });

    return this.service.updateGlobalChatSettings(validatedDto as UpdateGlobalChatSettingsDto);
  }

  @Post('cleanup-messages')
  @ApiOperation({ summary: 'Ручная очистка старых сообщений (админ)' })
  @ApiQuery({ name: 'retentionDays', required: false, type: Number, example: 30, description: 'Количество дней хранения (если не указано, используется настройка из глобальных настроек)' })
  @ApiResponse({ status: 200, description: 'Очистка выполнена', schema: { type: 'object', properties: { deleted: { type: 'number' } } } })
  async cleanupMessages(@Query('retentionDays') retentionDays?: number) {
    const result = await this.cleanupService.cleanupMessagesManually(retentionDays);
    return result;
  }
}
