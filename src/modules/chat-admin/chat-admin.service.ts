...
export class ChatAdminService {
  ...
  async getUnifiedCommunitiesList({ page = 1, limit = 100, type }: { page?: number; limit?: number; type?: 'COMMUNITY' | 'EVENT' }): Promise<{ data: UnifiedCommunityDto[]; total: number; page: number; limit: number }> {
    const skip = (page - 1) * limit;

    if (type === 'COMMUNITY') {
      const communityIdsWithMessages = await (this.prisma as any).communityMessage.groupBy({
        by: ['communityId'],
      ...
      const [communities, total] = await Promise.all([
        this.prisma.community.findMany({
          ...
          select: { id: true, name: true, status: true, isActive: true, createdAt: true },
      ...
      const data: UnifiedCommunityDto[] = communityCounts.map(({ community: c, messageCount, participantCount }) => ({
        id: c.id,
        name: c.name,
    ...
    if (type === 'EVENT') {
      const eventIdsWithMessages = await (this.prisma as any).eventMessage.groupBy({
        by: ['eventId'],
        where: { isDeleted: false },
      });
      ...
      const [events, total] = await Promise.all([
        this.prisma.event.findMany({
          ...
          select: { id: true, title: true, isActive: true, createdAt: true, communityId: true },
          orderBy: { createdAt: 'desc' },
          skip,
          take: limit,
        }),
      ...
      const data: UnifiedCommunityDto[] = eventCounts.map(({ event: e, messageCount, participantCount }) => ({
        id: e.id,
        name: e.title,
        type: 'EVENT' as const,
    ...
    const communityData: UnifiedCommunityDto[] = communityCounts.map(({ community: c, messageCount, participantCount }) => ({
      id: c.id,
      name: c.name,
      type: 'COMMUNITY' as const,
      status: c.status,
    ...
    const eventData: UnifiedCommunityDto[] = eventCounts.map(({ event: e, messageCount, participantCount }) => ({
      id: e.id,
      name: e.title,
      type: 'EVENT' as const,
      status: (e.isActive ? 'ACTIVE' : 'INACTIVE') as 'ACTIVE' | 'INACTIVE',
    ...
    const merged = [...communityData, ...eventData]
  ...
  async updateGlobalChatSettings(dto: UpdateGlobalChatSettingsDto): Promise<GlobalChatSettingsResponseDto> {
  ...
  async getModerationMessages(query: MessagesQueryDto): Promise<{ data: ModerationMessageDto[]; total: number; page: number; limit: number }> {
    ...
    if (chatType === AdminChatType.COMMUNITY) {
      ...
      const data: ModerationMessageDto[] = (rows as any[]).map((m: any) => ({
        ...
        eventId: null,
        userId: m.userId,
        userName: [m.user?.firstName, m.user?.lastName].filter(Boolean).join(' ') || '—',
  ...
  async softDeleteMessage(id: number | string, chatType?: 'COMMUNITY' | 'EVENT'): Promise<{ success: boolean; id: number }> {
    ...
    if (chatType === 'COMMUNITY') {
      const communityResult = await (this.prisma as any).communityMessage.updateMany({
        where: { 
          id: messageId,
          isDeleted: false
        },
      ...
      throw new NotFoundException('Community message not found or already deleted');
    }
  ...
  async approveMessage(messageId: number): Promise<void> {
    ...
    const cm = await (this.prisma as any).communityMessage.findUnique({ where: { id: messageId }, select: { id: true } });
    if (cm) {
      await (this.prisma as any).communityMessage.update({ where: { id: messageId }, data: { isModerated: true } });
      return;
    }

    const em = await (this.prisma as any).eventMessage.findUnique({ where: { id: messageId }, select: { id: true } });
    if (em) {
      await (this.prisma as any).eventMessage.update({ where: { id: messageId }, data: { isModerated: true } });
      return;
    }
  ...
  async rejectMessage(messageId: number): Promise<void> {
    ...
    const cm = await (this.prisma as any).communityMessage.findUnique({ where: { id: messageId }, select: { id: true } });
    if (cm) {
      await (this.prisma as any).communityMessage.update({ where: { id: messageId }, data: { isDeleted: true } });
      return;
    }

    const em = await (this.prisma as any).eventMessage.findUnique({ where: { id: messageId }, select: { id: true } });
    if (em) {
      await (this.prisma as any).eventMessage.update({ where: { id: messageId }, data: { isDeleted: true } });
      return;
    }
