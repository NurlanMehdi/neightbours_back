...
export class AiApiService {
  ...
  constructor(
    ...
    this.aiApiKey =
      ...
      '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';
  ...
  async sendMessage(
    ...
    this.logger.log(
      `Отправка сообщения в OpenRouter API: ${message.substring(0, 100)}...`,
    );
    ...
    try {
      const messages: OpenRouterMessage[] = [
        {
          role: 'system',
          content:
            'Ты полезный помощник для приложения соседского сообщества. Отвечай на русском языке, будь дружелюбным и помогай пользователям с их вопросами о жизни в сообществе, недвижимости, мероприятиях и общении с соседями.',
        },
        {
          role: 'user',
          content: message,
      ...
      this.logger.log(
        `Получен ответ от OpenRouter API: ${response.data.choices[0]?.message.content.substring(0, 100)}...`,
      );
      ...
      if (error.response) {
        ...
        const message =
          error.response.data?.error?.message || 'Ошибка AI сервиса';

        if (status === 401) {
          throw new HttpException(
            'Неверный API ключ для AI сервиса',
            HttpStatus.FORBIDDEN,
          );
        } else if (status === 400) {
          throw new HttpException(
            'Некорректный запрос к AI сервису',
  ...
  async getApiInfo(): Promise<any> {
    try {
      ...
      this.logger.error(
        'Ошибка при получении информации о OpenRouter API:',
        error.message,
      );
      throw new HttpException(
        'Не удалось получить информацию о AI сервисе',
        HttpStatus.SERVICE_UNAVAILABLE,
      );
    }
  }
