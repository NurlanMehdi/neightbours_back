...
export class NotificationService implements INotificationService {
  ...
  async createNotification(
    ...
    if (data.type === NotificationType.MESSAGE_RECEIVED) {
      ...
      this.logger.log(
        `[NotificationService] Created MESSAGE_RECEIVED for user ${data.userId}, message ${messageId}`,
      );
    }
  ...
  async createMultipleNotifications(
    ...
    notifications.forEach((n, idx) => {
      if (n.type === NotificationType.MESSAGE_RECEIVED) {
        ...
        this.logger.log(
          `[NotificationService] Created MESSAGE_RECEIVED for user ${n.userId}, message ${messageId}`,
        );
      }
    });
  ...
  async createGlobalNotification(data: IGlobalNotificationData): Promise<void> {
    ...
    if (
      ...
      this.logger.warn(
        'MESSAGE_RECEIVED тип не найден в enum, пропускаем уведомление',
      );
      return;
    }
  ...
  private async sendBulkPushNotifications(
    ...
    for (const user of users) {
      ...
      if (userNotifications && user.fcmToken) {
