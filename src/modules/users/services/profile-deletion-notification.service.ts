...
export class ProfileDeletionNotificationService {
  ...
  async checkDeletionSchedules(): Promise<void> {
    ...
    try {
      const usersWithScheduledDeletion = await this.profileDeletionRepository.findUsersWithDeletionScheduled();
      ...
      for (const user of usersWithScheduledDeletion) {
        ...
        if (daysLeft === 7) {
          ...
          this.logger.log(`Отправлено предупреждение пользователю ${user.id}: 7 дней до удаления`);
        } else if (daysLeft === 3) {
          this.profileDeletionGateway.notifyDeletionWarning(user.id, 3);
          this.logger.log(`Отправлено предупреждение пользователю ${user.id}: 3 дня до удаления`);
        } else if (daysLeft === 1) {
          this.profileDeletionGateway.notifyDeletionWarning(user.id, 1);
          this.logger.log(`Отправлено предупреждение пользователю ${user.id}: 1 день до удаления`);
        } else if (hoursLeft <= 24 && hoursLeft > 0) {
          this.profileDeletionGateway.notifyDeletionUrgent(user.id, hoursLeft);
          this.logger.log(`Отправлено срочное уведомление пользователю ${user.id}: ${hoursLeft} часов до удаления`);
        }
      }
    } catch (error) {
      this.logger.error(`Ошибка при проверке расписания удаления: ${error.message}`);
    }
  }

  async notifyDeletionCancelled(userId: number): Promise<void> {
    this.profileDeletionGateway.notifyDeletionCancelled(userId);
    this.logger.log(`Отправлено уведомление об отмене удаления пользователю ${userId}`);
  }

  async notifyDeletionCompleted(userId: number): Promise<void> {
    this.profileDeletionGateway.notifyDeletionCompleted(userId);
    this.logger.log(`Отправлено уведомление о завершении удаления пользователю ${userId}`);
  }
}
