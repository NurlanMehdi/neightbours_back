...
export class EventsController {
  ...
  @ApiOperation({
    ...
    description:
      'Отправляет сообщение в чат мероприятия. Поведение зависит от глобальных настроек чата: если чаты событий отключены, возвращает 403 Forbidden. Если включена модерация, сообщение будет ожидать одобрения администратора.',
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiResponse({
    status: 403,
    description:
      'Пользователь не является участником мероприятия или чаты событий отключены администратором',
  })
  @ApiResponse({
    status: 404,
    description: 'Мероприятие не найдено',
